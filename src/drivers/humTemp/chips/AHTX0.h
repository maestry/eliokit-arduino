 ...
 *
 */
class AHTX0_Temp : public Adafruit_Sensor {
public:
  /** @brief Create an Adafruit_Sensor compatible object for the temp sensor
      @param parent A pointer to the AHTX0 class */
  AHTX0_Temp(AHTX0 *parent) { _theAHTX0 = parent; }
...
protected:
  ...
  AHTX0_Temp *temp_sensor = NULL; ///< Temp sensor data object
  AHTX0_Humidity *humidity_sensor =
      NULL; ///< Humidity sensor data object

private:
  void _fetchTempCalibrationValues(void);
  void _fetchHumidityCalibrationValues(void);
  friend class AHTX0_Temp;     ///< Gives access to private members to
                                        ///< Temp data object
  friend class AHTX0_Humidity; ///< Gives access to private members to
